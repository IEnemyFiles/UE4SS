...
namespace RC::Unreal
{
    ...
    public:
        ...
        FORCEINLINE void CheckAddress(const ElementType* Addr) const
        {
            ...
            {
                Output::send<LogLevel::Error>(std::format(STR("Attempting to use a container element ({}) which already comes from the container being modified ({}, ArrayMax: {}, ArrayNum: {}, TypeSize: {})!"), static_cast<const void*>(Addr), static_cast<const void*>(GetData()), ArrayMax, ArrayNum, GetTypeSize()));
            }
        }
    ...
    private:
        void RemoveAtImpl(SizeType Index, SizeType Count, bool bAllowShrinking)
        {
            if (Count)
            {
                ...
                {
                    Output::send<LogLevel::Error>(STR("Unable to remove from array.  Count or index are less than 0 or exceed total array number.\n"));
                    return;
                }
    ...
    private:
        void RemoveAtSwapImpl(SizeType Index, SizeType Count = 1, bool bAllowShrinking = true)
        {
            if (Count)
            {
                ...
                if (!((Count >= 0) & (Index >= 0) & (Index + Count <= ArrayNum)))
                {
                    Output::send<LogLevel::Error>(STR("Unable to remove from array.  Count or index are less than 0.\n"));
                    return;
                }
