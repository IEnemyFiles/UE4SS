...
std::function<void(int32)>*& UEnum::GetEnumDisplayNameFn()
{
    ...
    return *Helper::Casting::ptr_cast<std::function<void(int32)>**>(this, offset->second);
}
const std::function<void(int32)>*& UEnum::GetEnumDisplayNameFn() const
{
    static auto offset = MemberOffsets.find(STR("EnumDisplayNameFn"));
    if (offset == MemberOffsets.end()) { throw std::runtime_error{"Tried getting member variable 'UEnum::EnumDisplayNameFn' that doesn't exist in this engine version."}; }
    return *Helper::Casting::ptr_cast<const std::function<void(int32)>**>(this, offset->second);
}
