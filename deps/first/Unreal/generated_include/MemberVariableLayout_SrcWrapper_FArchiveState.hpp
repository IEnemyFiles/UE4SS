...
uint8& FArchiveState::GetArShouldSkipUpdateCustomVersion()
{
    static auto& offsets = Version::IsBelow(4, 25) ? FArchive::MemberOffsets : FArchiveState::MemberOffsets;
    static auto offset = offsets.find(STR("ArShouldSkipUpdateCustomVersion"));
    if (offset == offsets.end()) { throw std::runtime_error{"Tried getting member variable 'FArchiveState::ArShouldSkipUpdateCustomVersion' that doesn't exist in this engine version."}; }
    return *Helper::Casting::ptr_cast<uint8*>(this, offset->second);
}
const uint8& FArchiveState::GetArShouldSkipUpdateCustomVersion() const
{
    static auto& offsets = Version::IsBelow(4, 25) ? FArchive::MemberOffsets : FArchiveState::MemberOffsets;
    static auto offset = offsets.find(STR("ArShouldSkipUpdateCustomVersion"));
    if (offset == offsets.end()) { throw std::runtime_error{"Tried getting member variable 'FArchiveState::ArShouldSkipUpdateCustomVersion' that doesn't exist in this engine version."}; }
