...
namespace RC::Unreal::UObjectGlobals
{
    ...
    auto StaticConstructObject(const FStaticConstructObjectParameters& Params) -> UObject*
    {
        ...
        {
            return GlobalState::StaticConstructObjectInternal(Params);
        }
    }
    ...
    UObject* FindObject(UClass* Class, UObject* InOuter, const TCHAR* InName, bool bExactClass, ObjectSearcher* InSearcher)
    {
        ...
        {
            if (auto CachedObject = GetGlobalObject(InName); CachedObject)
            {
                return CachedObject;
            }
        }
