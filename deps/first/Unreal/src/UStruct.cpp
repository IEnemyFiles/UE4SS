...
#include <MemberVariableLayout_SrcWrapper_UStruct.hpp>
    ...
    auto UStruct::OrderedForEachPropertyInChain() -> Generator<FProperty*>
    {
      UStruct* Class = this;
      std::vector<UStruct*> SuperStructs{ this };
      ...
      while (Class)
      {
        for (UStruct* SuperStruct : Class->ForEachSuperStruct())
        {
          ...
          SuperStructs.emplace_back(SuperStruct);
        }
      ...
      for (auto SuperStruct : std::views::reverse(SuperStructs))
      {
        for (FProperty* Property : SuperStruct->ForEachProperty())
        {
          co_yield Property;
