        ...
        if (Version::IsAtLeast(4, 25))
        {
            auto find_all_property_types = [](const std::wstring& obj_string) -> void {
                ...
                while (field)
                {
                    ...
                    if (type_name == FName(L"SetProperty"))
                    {
            ...
            find_all_property_types(L"/Script/VariantManagerContent.VariantObjectBinding");
            ...
            find_all_property_types(L"/Script/MovieSceneTracks.MovieSceneEventPtrs");
        }
        else
        {
